...
export const SummaryReport: FC<SummaryReportProps> = ({
  ...
  const selectedSuggestions: string[] = Array.isArray(data.selectedSuggestions)
  ...
  const renderSystemReadingsSection = () => {
    ...
    return (
      <div className="summary-section">
        ...
        <div className="report-readings-grid">
          <div className="report-readings-card">
            <h3>Furnace / Air Handler</h3>
            {renderReading('Blower Capacitor', systemReadings.blowerCapacitor)}
            {renderReading('Blower Amps', systemReadings.blowerAmps)}
            {renderReading('Inducer Motor Amps', systemReadings.inducerMotorAmps)}
            {renderReading('Gas Pressure', systemReadings.gasPressure)}
            {renderReading('Temperature Rise', systemReadings.temperatureRise)}
            {renderReading('Return Air Temp', systemReadings.returnAirTemp)}
            {renderReading('Supply Air Temp', systemReadings.supplyAirTemp)}
          </div>

          <div className="report-readings-card">
            <h3>AC / Heat Pump</h3>
            {renderReading('Outdoor Temp', systemReadings.outdoorTemp)}
            {renderReading('Indoor Wet Bulb', systemReadings.indoorWetBulb)}
            {renderReading('Low Side Pressure', systemReadings.lowSidePressure)}
            {renderReading('High Side Pressure', systemReadings.highSidePressure)}
            {renderReading('Superheat', systemReadings.superheat)}
            {renderReading('Subcooling', systemReadings.subcooling)}
            {renderReading('Compressor Amps', systemReadings.compressorAmps)}
            {renderReading('Condenser Fan Amps', systemReadings.condenserFanAmps)}
            {renderReading('Capacitor Herm', systemReadings.capacitorHerm)}
            {renderReading('Capacitor Fan', systemReadings.capacitorFan)}
            {renderReading('Capacitor Common', systemReadings.capacitorCommon)}
          </div>
        </div>
      </div>
    )
      ...
      {incompleteItems.length > 0 ? (
        <div className="issues-list">
          {sortedIncompleteItems.map((item: InspectionItem) => {
            ...
            return (
              <div key={item.id || item.itemName} className="issue-item">
                <div className="issue-header">
                  <div className="issue-title">{primary || '—'}</div>
  ...
  const completeAndUploadToDashboard = async () => {
    try {
      ...
      const publicUrl = supabase.storage.from('service-docs').getPublicUrl(filePath)?.data?.publicUrl
      ...
      const { error: insertErr } = await supabase.from('service_docs').insert(insertPayload)
  ...
  return (
    <div className="summary-report">
              ...
              {sortedSuggestions.map((suggestion: any, idx: number) => (
                <div key={idx} className="recommendation-card">
                  <div className="recommendation-header">
                    <div className="recommendation-title">{suggestion?.title ?? 'Add-On'}</div>
      ...
      <div className="summary-actions">
        ...
        <button onClick={onExportPDF} className="btn btn-primary" type="button">
          Export PDF
        </button>
